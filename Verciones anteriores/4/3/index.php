<?php 
    ...
    //neutro
    $tweets = $twitter->get('https://api.twitter.com/1.1/search/tweets.json?q=lopezobrador_&result_type=recent&count=50');
  /* //negativo
    $tweets2 = $twitter->get('https://api.twitter.com/1.1/search/tweets.json?q=amlo&result_type=recent&count=10');
    //positivo
	$tweets3= $twitter->get('https://api.twitter.com/1.1/search/tweets.json?q=te_queremos_amlo&type=recent&count=10');*/
	$data = array();
    $extraer = 45;
  ...
  /*  foreach ($tweets2->statuses as $key => $tweet)
	...
	for($i = 0 ; $i < $extraer; $i++)
	{
		...
		//echo ($data[$i][3]."  |  ".exec("temp.py $text"))."<br>";
    ...
    <body>
        <table border="1">
            ...
            <tr>
                ...
                <td> <h3>TOTAL DEL TWEETS EXRAIDOS = <?php echo $extraer ?> <br> RANGO DE FECHAS = <br>TOTAL DE TWEETS POSITIVOS =<?php echo $n_p?> <br> TOTAL DE TWEETS NEGATIVOS = <?php echo $n_n?> <br> ESTADO DEL STATUS =<?php echo $status?> <br> </h3> </td>
